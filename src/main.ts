...
let optimizer:Worker|null=null,optimizerStopSignal:Int32Array|null=null,runVersion=0,optimizing=false,stoppingRequested=false,runStarted=0,runFinished=0;
let presenter:Worker|null=null,presentationBusy=false,presentationSequence=0;
let pendingPresentation:Extract<OptimizerEvent,{type:"discoverySnapshot"}>|null=null;
let validationCompleted=0,validationTotal=1;
...
let certifying=false,certificationId=0;
  type CertificationJob={candidateId:number;source:"baseline";queueKey:string;genotype:Float64Array;provisionalLapTime:number};
  let certificationQueue:CertificationJob[]=[],activeCertification:CertificationJob|null=null;
  let optimizerStopped=false,runCertifiedImprovement=false;
...
function validationPercent():number{return Math.round(100*Math.max(0,Math.min(validationCompleted,validationTotal))/Math.max(1,validationTotal));}
function setValidationProgress(completed:number,total:number,label:string):void{validationCompleted=completed;validationTotal=Math.max(1,total);const progress=$("work-overlay").querySelector<HTMLElement>(".validation-progress")!,bar=progress.querySelector<HTMLElement>("i")!,percent=validationPercent();progress.setAttribute("aria-valuenow",String(percent));progress.setAttribute("aria-valuetext",`${label} · ${percent}%`);bar.style.width=`${percent}%`;}
...
function certifiedIncumbentLap():number{return currentCertificate?currentProfile.lapTime:Infinity;}
function elapsedClock(elapsedMs:number):string{const seconds=Math.max(0,Math.floor(elapsedMs/1000)),minutes=Math.floor(seconds/60);return`${minutes}:${String(seconds%60).padStart(2,"0")}`;}

function stopPresentationWorker():void{const worker=presenter;presenter=null;pendingPresentation=null;presentationBusy=false;presentationSequence=0;if(worker)terminateWorker(worker);}
function ensurePresentationWorker():Worker{
  if(presenter)return presenter;
  const ownerVersion=runVersion,worker=new Worker(new URL("./workers/presentation-worker.ts",import.meta.url),{type:"module"});
  presenter=worker;
  worker.onmessage=(event:MessageEvent<PresentationEvent>)=>{if(presenter!==worker||runVersion!==ownerVersion)return;handlePresentation(event.data,worker);};
  worker.onerror=event=>{if(presenter!==worker||runVersion!==ownerVersion)return;event.preventDefault();const message=nativeWorkerError(event,"live presentation worker failed without a diagnostic");presenter=null;presentationBusy=false;terminateWorker(worker);setStatus(`Optimization continues · live presentation unavailable · ${message}`,"busy");};
  worker.onmessageerror=()=>{if(presenter!==worker||runVersion!==ownerVersion)return;presenter=null;presentationBusy=false;terminateWorker(worker);setStatus("Optimization continues · live presentation message could not be decoded","busy");};
  worker.postMessage({...messageEnvelope(),type:"init",compiledTrack:track,vehicle:{...vehicle}} satisfies PresentationCommand);
  return worker;
}
function pumpPresentation():void{if(presentationBusy||pendingPresentation===null||!optimizing||stoppingRequested)return;const job=pendingPresentation;pendingPresentation=null;presentationBusy=true;presentationSequence=job.sequence;const worker=ensurePresentationWorker();const command:PresentationCommand={...messageEnvelope(),type:"prepareLiveProduct",sequence:job.sequence,elapsedMs:job.elapsedMs,optimizerLapTime:job.optimizerLapTime,candidateId:job.candidateId,basis:job.basis,corridor:job.corridor,sources:job.sources};worker.postMessage(command,job.sources.map(source=>source.buffer));}
function queuePresentation(event:Extract<OptimizerEvent,{type:"discoverySnapshot"}>):void{if(!optimizing||stoppingRequested)return;if(event.sequence<=presentationSequence)return;if(pendingPresentation===null||event.sequence>pendingPresentation.sequence)pendingPresentation=event;pumpPresentation();}
function handlePresentation(event:PresentationEvent,source:Worker):void{if(source!==presenter||event.runVersion!==runVersion||event.trackFingerprint!==fingerprint||event.settingsFingerprint!==settingsFingerprint()||event.sequence!==presentationSequence)return;if(event.type==="presentationProgress"){if(optimizing&&!stoppingRequested)setStatus(`Optimizing · preparing ${elapsedClock(30_000*event.sequence)} certified trajectory · ${Math.round(100*event.completed/Math.max(1,event.total))}%`,"busy");return;}presentationBusy=false;if(event.type==="liveProductCertified"){if(optimizing&&!stoppingRequested&&event.certificate.pass){if(shouldAdoptCertifiedLap(event.lapTime,certifiedIncumbentLap())){commitCurvature(event);setStatus(`Optimizing · ${elapsedClock(event.elapsedMs)} certified live ${event.lapTime.toFixed(3)} s`,"busy");}else setStatus(`Optimizing · ${elapsedClock(event.elapsedMs)} certified check retained ${currentProfile.lapTime.toFixed(3)} s`,"busy");}}else if(optimizing&&!stoppingRequested)setStatus(`Optimization continues · ${elapsedClock(30_000*event.sequence)} presentation rejected · ${event.message}`,"busy");pumpPresentation();}
...
function renderSettings():void{const grid=$("settings-grid");grid.replaceChildren();for(let index=0;index<SETTINGS.length;index++){const spec=SETTINGS[index]!,wrap=document.createElement("div"),domainChanged=index>0&&SETTINGS[index-1]!.domain!==spec.domain;wrap.className=`setting${domainChanged?" constraint-domain-start":""}`;wrap.dataset.constraintDomain=spec.domain;wrap.style.setProperty("--constraint-color",CONSTRAINT_COLORS[spec.domain]);const stored=vehicle[spec.key],value=stored===null?null:spec.toDisplay?.(stored)??stored;wrap.innerHTML=`<div class="setting-editor"><label for="setting-${spec.key}"><span class="setting-label">${spec.label}<span class="setting-comment"> — ${spec.comment}</span></span></label><div class="input-wrap"><input id="setting-${spec.key}" type="number" step="${spec.step}" ${spec.min===undefined?"":`min="${spec.min}"`} ${spec.max===undefined?"":`max="${spec.max}"`} value="${value===null?"":Number(value.toFixed(6))}"><span>${spec.unit}</span></div></div><div class="setting-equation"><span class="formula">${spec.equation}</span></div>`;
  ...
  const mode=document.createElement("div");mode.className="setting constraint-domain-start";mode.dataset.constraintDomain="none";mode.style.setProperty("--constraint-color",CONSTRAINT_COLORS.none);mode.innerHTML='<div class="setting-editor"><label for="setting-run-mode"><span class="setting-label">Run mode<span class="setting-comment"> — Every Optimize click starts a fresh search.</span></span></label><div class="input-wrap"><select id="setting-run-mode" class="run-mode"><option value="random">Nondeterministic</option><option value="deterministic">Deterministic</option></select></div></div><div class="setting-equation"><span class="formula">xₖ₊₁ = Φ(xₖ, ξₖ)</span></div>';const select=mode.querySelector("select")!;select.value=deterministicRun?"deterministic":"random";select.onchange=()=>{if(settingsLocked()){select.value=deterministicRun?"deterministic":"random";return;}deterministicRun=select.value==="deterministic";showToast(`${deterministicRun?"Deterministic":"Nondeterministic"} mode selected for the next run.`);};grid.append(mode);}

function renderProfileLabels():void{const host=$("profile-labels"),axes=document.createElement("div"),series=document.createElement("div");axes.className="chart-axis-labels";axes.style.flexBasis=`${PROFILE_CHART_AXIS_COLUMN_PX*PROFILE_CHART_LABELS.length}px`;axes.style.gridTemplateColumns=`repeat(${PROFILE_CHART_LABELS.length}, ${PROFILE_CHART_AXIS_COLUMN_PX}px)`;series.className="chart-series-labels";for(const label of PROFILE_CHART_LABELS){const axis=document.createElement("span"),legend=document.createElement("span");axis.className="chart-axis-label";legend.className="chart-series-label";axis.style.color=label.color;legend.style.color=label.color;axis.textContent=`${label.shortName} · ${label.unit}`;legend.textContent=label.name;axes.append(axis);series.append(legend);}host.replaceChildren(axes,series);}
...
function recertifyCenter(status:string):void{stopOptimizer(true);optimizerStopped=false;runVersion++;resetCertifier();playing=false;pausedRaceTime=0;profileCursorTime=0;profileMarkActive=false;currentLine=centerlineSpec(track);currentGenotype=new Float64Array(64);currentCertificate=null;currentV2Representations=null;scene.certifiedBest=null;scene.provisionalBest=null;scene.invalidFlash=false;scene.candidateLines=null;scene.candidateOffsets=null;for(const id of ["station-rate","candidate-count"])$(id).textContent="—";rebuildProfile();setStatus(status,"busy");queueBaselineCertification(currentGenotype,currentProfile.lapTime);}

function resetSettings():void{if(settingsLocked())return;vehicle={...DEFAULT_VEHICLE};deterministicRun=false;activeSeedLo=0x12345678;activeSeedHi=0x9abcdef0;runStarted=0;runFinished=0;updateRunElapsed();renderSettings();recertifyCenter("Validating default settings");showToast("Vehicle settings, run mode, and optimization time reset.");}
...
function queueBaselineCertification(genotype:Float64Array,provisionalLapTime:number):void{if(!optimizing)setValidationProgress(0,1,"Checking binary64 centerline");enqueueCertification({candidateId:++certificationId,source:"baseline",queueKey:"baseline",genotype:new Float64Array(genotype),provisionalLapTime});}

async function pumpCertification():Promise<void>{if(activeCertification||certificationQueue.length===0){finishStoppedRun();return;}const job=certificationQueue.shift()!;activeCertification=job;certifying=true;try{await ensureCertifier();if(activeCertification!==job)return;certifier!.postMessage({...messageEnvelope(),type:"certifyCenterline",compiledTrack:track,vehicle:{...vehicle},provisionalLapTime:job.provisionalLapTime,candidateId:job.candidateId} satisfies CertifierCommand);}catch(error){if(activeCertification!==job)return;activeCertification=null;const message=errorText(error,"certification failed without a diagnostic");setStatus(currentCertificate?`Certification skipped · ${message}; retained ${certifiedIncumbentLap().toFixed(3)} s`:`Certifier error · ${message}`,currentCertificate?"active":"idle");void pumpCertification();}}

function commitBaseline(event:Extract<CertifierEvent,{type:"centerlineCertified"}>):void{const line=centerlineSpec(track);currentLine=line;currentGenotype=new Float64Array(64);currentCertificate=event.certificate;currentV2Representations=null;currentProfile=profileFromPacked(event.profileNodes,event.edgeCount,event.lapTime,line);profileCursorTime=0;pausedRaceTime=0;profileMarkActive=false;scene.certifiedBest=null;scene.provisionalBest=null;scene.invalidFlash=false;updateProfileDisplay();}

type CurvatureCommit={genotype:Float64Array;lapTime:number;lineLengthM:number;profileNodes:Float64Array;edgeCount:number;pathSamples:Float64Array;representations:V2RepresentationsJson;certificate:CertificateReportJson};
function commitCurvature(event:CurvatureCommit):void{const priorLap=certifiedIncumbentLap(),line=curvatureLineFromSamples(event.pathSamples,event.lineLengthM);currentLine=line;currentGenotype=new Float64Array(event.genotype);currentCertificate=event.certificate;currentV2Representations=event.representations;currentProfile=profileFromPacked(event.profileNodes,event.edgeCount,event.lapTime,line,event.lineLengthM);profileCursorTime=0;pausedRaceTime=0;profileMarkActive=false;scene.certifiedBest=line;scene.provisionalBest=null;scene.invalidFlash=false;updateProfileDisplay();if(event.lapTime<priorLap-1e-6)runCertifiedImprovement=true;}

function finishStoppedRun():void{certifying=activeCertification!==null||certificationQueue.length>0;syncButtons();if(!optimizerStopped||certifying)return;if(runFinished===0)runFinished=performance.now();if(runCertifiedImprovement)setStatus(`Stopped · certified best ${currentProfile.lapTime.toFixed(3)} s`,"active");else if(currentCertificate)setStatus(`Stopped · no certified improvement; retained ${currentProfile.lapTime.toFixed(3)} s`,"active");else setStatus("Stopped · no feasible candidate","active");updateRunElapsed();syncButtons();}
...
function startOptimizer():void{
  if(optimizing||trackSwitching)return;
  stopOptimizer(true);runVersion++;resetCertifier();
  ...
  const centerGenotype=new Float64Array(64),seedGenotype=currentCertificate?currentGenotype:centerGenotype,centerProfile=evaluateProfile(centerlineSpec(track),vehicle);
  optimizing=true;stoppingRequested=false;optimizerStopped=false;runCertifiedImprovement=false;runStarted=performance.now();runFinished=0;
  const candidateCount=$("candidate-count");candidateCount.textContent="0";delete candidateCount.dataset.totalCandidates;scene.provisionalBest=null;scene.invalidFlash=false;scene.candidateLines=null;scene.candidateOffsets=null;if(!currentCertificate)queueBaselineCertification(centerGenotype,centerProfile.lapTime);
  const ownerVersion=runVersion;let worker:Worker;
  try{worker=new Worker(new URL("./workers/optimizer-worker.ts",import.meta.url),{type:"module"});}
  catch(error){runFinished=performance.now();optimizing=false;setStatus(`Optimizer error · ${errorText(error,"optimizer worker could not be created")}`,"idle");syncButtons();return;}
  optimizer=worker;
  worker.onmessage=(event:MessageEvent<OptimizerEvent>)=>{if(optimizer!==worker||runVersion!==ownerVersion)return;handleOptimizer(event.data,worker);};
  worker.onerror=event=>{if(optimizer!==worker||runVersion!==ownerVersion)return;event.preventDefault();const message=nativeWorkerError(event,"optimizer worker failed without a diagnostic");runFinished=performance.now();setStatus(`Optimizer error · ${message}`,"idle");stopOptimizer(true);};
  worker.onmessageerror=()=>{if(optimizer!==worker||runVersion!==ownerVersion)return;runFinished=performance.now();setStatus("Optimizer error · worker message could not be decoded","idle");stopOptimizer(true);};
  ...
  const stopBuffer=globalThis.crossOriginIsolated&&typeof SharedArrayBuffer!=="undefined"?new SharedArrayBuffer(Int32Array.BYTES_PER_ELEMENT):null;
  optimizerStopSignal=stopBuffer===null?null:new Int32Array(stopBuffer);
  const envelope=messageEnvelope();
  const init:OptimizerCommand={...envelope,type:"init",compiledTrack:track,vehicle:{...vehicle},optimizer:{seedLo:activeSeedLo,seedHi:activeSeedHi,deterministic:deterministicRun,candidateVisibility:8},stopSignal:stopBuffer};
  worker.postMessage(init);
  worker.postMessage({...envelope,type:"start",seedGenotype,checkpoint:null} satisfies OptimizerCommand);
  setStatus(`GPU optimization running until STOP · ${deterministicRun?"deterministic":"fresh-seed"} restarts`,"busy");updateRunElapsed();syncButtons();
}

function handleOptimizer(event:OptimizerEvent,source:Worker):void{if(source!==optimizer||event.runVersion!==runVersion||event.trackFingerprint!==fingerprint||event.settingsFingerprint!==settingsFingerprint())return;if(event.type==="ready"){if(event.adapterInfo!=="initializing"){const seed=`${activeSeedLo.toString(16).padStart(8,"0")}:${activeSeedHi.toString(16).padStart(8,"0")}`;setStatus(`${event.cpuFallback?"CPU fallback":"GPU"} · ${event.adapterInfo} · runs until STOP · ${deterministicRun?"deterministic":`seed ${seed}`}`,"busy");}}
  else if(event.type==="progress"){$("station-rate").textContent=Math.round(event.throughput.stationPerSecond).toLocaleString();const candidateCount=$("candidate-count");candidateCount.textContent=event.candidates.toLocaleString();candidateCount.dataset.totalCandidates=String(event.candidates);}
  else if(event.type==="displayCandidates"){scene.candidateLines=event.lines;scene.candidateOffsets=event.lineOffsets;}
  else if(event.type==="discoverySnapshot"){queuePresentation(event);}
  else if(event.type==="warning"){setStatus(`${event.stage} stage skipped · ${event.message} · retained canonical incumbent`,"busy");}
  else if(event.type==="stopped"){stopPresentationWorker();optimizing=false;stoppingRequested=false;optimizerStopped=true;scene.candidateLines=null;scene.candidateOffsets=null;optimizer=null;optimizerStopSignal=null;terminateWorker(source);setStatus(certifying?"Validating current trajectory":"Finalizing run","busy");finishStoppedRun();syncButtons();}
...
function stopOptimizer(immediate=false):void{const worker=optimizer;if(!worker){if(immediate){stopPresentationWorker();optimizerStopSignal=null;optimizing=false;stoppingRequested=false;syncButtons();}return;}if(immediate){if(optimizerStopSignal)Atomics.store(optimizerStopSignal,0,1);stopPresentationWorker();optimizer=null;optimizerStopSignal=null;terminateWorker(worker);optimizing=false;stoppingRequested=false;optimizerStopped=false;scene.candidateLines=null;scene.candidateOffsets=null;syncButtons();}else if(!stoppingRequested){stoppingRequested=true;stopPresentationWorker();if(optimizerStopSignal)Atomics.store(optimizerStopSignal,0,1);worker.postMessage({runVersion,trackFingerprint:fingerprint,settingsFingerprint:JSON.stringify(vehicle),type:"stop"} satisfies OptimizerCommand);setStatus("Stopping · retaining displayed certified trajectory","busy");syncButtons();}}

function syncButtons():void{const action=$("optimize-button") as HTMLButtonElement,locked=settingsLocked();action.textContent=optimizing?"STOP":"OPTIMIZE";action.disabled=stoppingRequested||editBusy||trackSwitching||(!optimizing&&!currentCertificate);action.classList.toggle("optimize",!optimizing);action.classList.toggle("stop",optimizing);($("track-list").querySelectorAll("button") as NodeListOf<HTMLButtonElement>).forEach(b=>b.disabled=optimizing||editBusy||trackSwitching);($("new-track-button") as HTMLButtonElement).disabled=optimizing||editBusy||trackSwitching;($("track-editor-name") as HTMLInputElement).disabled=optimizing||editBusy||trackSwitching;($("track-editor-done") as HTMLButtonElement).disabled=optimizing||editBusy||trackSwitching;($("reset-button") as HTMLButtonElement).disabled=locked;($("settings-grid").querySelectorAll("input, select") as NodeListOf<HTMLInputElement|HTMLSelectElement>).forEach(control=>control.disabled=locked);($("play-button") as HTMLButtonElement).disabled=false;($("save-button") as HTMLButtonElement).disabled=!currentCertificate||currentLine.kind!=="curvature"||currentV2Representations===null||optimizing||certifying||trackSwitching;$("play-button").textContent=playing?"Ⅱ PAUSE":"▶ PLAY";$("zoom-button").setAttribute("aria-pressed",String(zoomed));const overlay=$("work-overlay"),validating=!optimizing&&certifying,work=stoppingRequested?"STOPPING":validating?`VALIDATING ${validationPercent()}%`:optimizing?"OPTIMIZING":certifying?`VALIDATING ${validationPercent()}%`:"";overlay.hidden=work==="";overlay.classList.toggle("validating",validating);overlay.querySelector("span")!.textContent=work;scene.workLabel=null;}
...
async function saveCurrent():Promise<void>{if(!currentCertificate||currentLine.kind!=="curvature"||currentV2Representations===null){showToast("Only a certified canonical-curvature trajectory can be saved.");return;}const now=new Date(),id=crypto.randomUUID();const profile:SavedProfileJson={schemaVersion:2,profileId:id,name:`Run ${saved.length+1} · ${currentProfile.lapTime.toFixed(3)} s`,createdAt:now.toISOString(),trackId:track.source.id,trackFingerprint:fingerprint,vehicleSettings:{...vehicle},dynamicSettings:{seedLo:activeSeedLo,seedHi:activeSeedHi,deterministic:deterministicRun,candidateVisibility:8},optimizerSeed:[activeSeedLo,activeSeedHi],lineLengthM:currentProfile.lineLength,lapTimeS:currentProfile.lapTime,profileNodes:currentProfile.nodes,certificate:{...currentCertificate,hash:fingerprint},v2Representations:currentV2Representations};
  try{await saveProfileTransaction(profile);try{await navigator.storage?.persist?.();}catch{}await refreshSaved();showToast("Racing profile saved for future sessions on this site.");}catch(error){showToast(error instanceof Error?error.message:"Profile save failed");}}
