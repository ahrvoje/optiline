...
let deterministicRun=false,backgroundExecution=true,activeSeedLo=0x12345678,activeSeedHi=0x9abcdef0;
...
const savedLineCache=new Map<string,LineSpec>();
...
let certifying=false,certificationId=0;
  interface CertificationJob{candidateId:number;source:CandidateSource;queueKey:string;genotype:Float64Array;warmPreimage?:Float64Array;provisionalLapTime:number;representations?:V2RepresentationsJson;}
...
const SETTINGS:SettingSpec[]=[
  ...
  {key:"lengthM",label:"Rectangle length",unit:"m",step:"0.1",min:1,max:30,comment:"Physical swept-surface length.",equation:"Lₑ = Lᵥ + 2mₛ",domain:"containment"},
  {key:"widthM",label:"Rectangle width",unit:"m",step:"0.05",min:.5,max:12,comment:"Physical swept-surface width.",equation:"Wₑ = Wᵥ + 2mₛ",domain:"containment"},
  {key:"safetyMarginM",label:"Safety margin",unit:"m / side",step:"0.01",min:0,max:2,comment:"Added to every swept rectangle side.",equation:"Rₑ = R(Lₑ,Wₑ) ⊕ mₛ",domain:"containment"},
  ...
  {key:"airDensity",label:"Air density",unit:"kg/m³",step:"0.01",min:.5,max:1.5,comment:"Shared drag and downforce atmosphere.",equation:"q∞ = ½ρv²",domain:"aero"},
...
function curvatureLineFromSamples(pathSamples:Float64Array,pathLengthM:number,preimage:Float64Array,genotype:ArrayLike<number>):LineSpec{const shadow=racingLineFromPreimage(track,genotype,preimage);return{kind:"curvature",preimage:shadow.preimage,gates:shadow.gates,pathLengthM,samples:pathSamples};}
function lineFromSaved(profile:SavedProfileJson):LineSpec{const cached=savedLineCache.get(profile.profileId);if(cached)return cached;const preimage=flattenPairs(profile.preimageControls),shadow=racingLineFromPreimage(track,profile.genotypeD,preimage);let line:LineSpec=shadow;if(profile.v2Representations){try{const representation=curvatureRepresentationFromJson(profile.v2Representations.curvature),path=reconstructCurvaturePath(representation,4096),samples=new Float64Array(5*path.length);for(let i=0;i<path.length;i++){const sample=path[i]!;samples.set([sample.x,sample.y,sample.tx,sample.ty,sample.kappa],5*i);}line={kind:"curvature",preimage:shadow.preimage,gates:shadow.gates,pathLengthM:representation.pathLengthM,samples};}catch{line=shadow;}}savedLineCache.set(profile.profileId,line);return line;}
...
function renderSettings():void{const grid=$("settings-grid");grid.replaceChildren();for(const spec of SETTINGS){const wrap=document.createElement("div");wrap.className="setting";wrap.style.setProperty("--constraint-color",CONSTRAINT_COLORS[spec.domain]);const stored=vehicle[spec.key],value=stored===null?null:spec.toDisplay?.(stored)??stored;wrap.innerHTML=`<div class="setting-editor"><label for="setting-${spec.key}"><span>${spec.label}</span></label><div class="input-wrap"><input id="setting-${spec.key}" type="number" step="${spec.step}" ${spec.min===undefined?"":`min="${spec.min}"`} ${spec.max===undefined?"":`max="${spec.max}"`} value="${value===null?"":Number(value.toFixed(6))}"><span>${spec.unit}</span></div></div><div class="setting-equation"><span class="formula">${spec.equation}</span></div><p class="setting-description">${spec.comment}</p>`;
  ...
  const mode=document.createElement("div");mode.className="setting";mode.innerHTML='<div class="setting-editor"><label for="setting-run-mode"><span>Run mode</span></label><div class="input-wrap"><select id="setting-run-mode" class="run-mode"><option value="random">Nondeterministic</option><option value="deterministic">Deterministic</option></select></div></div><div class="setting-equation"><span class="formula">xₖ₊₁ = Φ(xₖ, ξₖ)</span></div><p class="setting-description">Every Optimize click starts a new worker and a fresh search state.</p>';const select=mode.querySelector("select")!;select.value=deterministicRun?"deterministic":"random";select.onchange=()=>{deterministicRun=select.value==="deterministic";showToast(`${deterministicRun?"Deterministic":"Nondeterministic"} mode selected for the next run.`);};grid.append(mode);
  const background=document.createElement("div");background.className="setting";background.innerHTML='<div class="setting-editor"><label for="setting-background-execution"><span>Background execution</span></label><div class="input-wrap"><select id="setting-background-execution" class="run-mode"><option value="on">Enabled</option><option value="off">Disabled</option></select></div></div><div class="setting-equation"><span class="formula">Δtᵧ = task(0)</span></div><p class="setting-description">Uses timer-free worker yields to resist hidden-tab throttling. Browser or OS suspension still applies.</p>';const backgroundSelect=background.querySelector("select")!;backgroundSelect.value=backgroundExecution?"on":"off";backgroundSelect.onchange=()=>{backgroundExecution=backgroundSelect.value==="on";if(optimizer)optimizer.postMessage({...messageEnvelope(),type:"setBackgroundExecution",enabled:backgroundExecution} satisfies OptimizerCommand);showToast(`Background execution ${backgroundExecution?"enabled":"disabled"}.`);};grid.append(background);}
...
function queueCertification(source:CandidateSource,genotype:Float64Array,provisionalLapTime:number,warmPreimage?:Float64Array,representations?:V2RepresentationsJson,queueKey:string=source):void{const job:CertificationJob={candidateId:++certificationId,source,queueKey,genotype:new Float64Array(genotype),provisionalLapTime,...(warmPreimage===undefined?{}:{warmPreimage:new Float64Array(warmPreimage)}),...(representations===undefined?{}:{representations})};certificationQueue=upsertPendingCandidate(certificationQueue,job);certifying=true;void pumpCertification();syncButtons();}

async function pumpCertification():Promise<void>{if(activeCertification||certificationQueue.length===0){finishStoppedRun();return;}const job=certificationQueue.shift()!;activeCertification=job;certifying=true;try{await ensureCertifier();if(activeCertification!==job)return;if(job.source==="curvature"&&job.representations&&job.warmPreimage){certifier!.postMessage({...messageEnvelope(),type:"certifyCurvature",compiledTrack:track,vehicle:{...vehicle},genotype:job.genotype,preimage:job.warmPreimage,representations:job.representations,provisionalLapTime:job.provisionalLapTime,candidateId:job.candidateId} satisfies CertifierCommand);}else{certifier!.postMessage({...messageEnvelope(),type:"certifyCandidate",compiledTrack:track,vehicle:{...vehicle},genotype:job.genotype,...(job.warmPreimage===undefined?{}:{warmPreimage:job.warmPreimage}),provisionalLapTime:job.provisionalLapTime,candidateId:job.candidateId} satisfies CertifierCommand);}}catch(error){if(activeCertification!==job)return;activeCertification=null;const message=errorText(error,"certification failed without a diagnostic");setStatus(currentCertificate?`Certification skipped · ${message}; retained ${currentProfile.lapTime.toFixed(3)} s`:`Certifier error · ${message}`,currentCertificate?"active":"idle");void pumpCertification();}}

function commitCertified(event:Extract<CertifierEvent,{type:"certified"}>,representations?:V2RepresentationsJson):void{const priorLap=currentCertificate?currentProfile.lapTime:Infinity;currentLine=lineFromCertified(event.preimage,event.genotype);currentGenotype=new Float64Array(event.genotype);currentCertificate=event.certificate;currentV2Representations=representations??null;currentProfile=profileFromPacked(event.profileNodes,event.edgeCount,event.lapTime,currentLine);profileCursorTime=0;pausedRaceTime=0;profileMarkActive=false;scene.certifiedBest=currentLine;scene.provisionalBest=null;scene.invalidFlash=false;updateProfileDisplay();if(event.lapTime<priorLap-1e-6)runCertifiedImprovement=true;}

function commitCurvature(event:Extract<CertifierEvent,{type:"curvatureCertified"}>):void{const priorLap=currentCertificate?currentProfile.lapTime:Infinity;currentLine=curvatureLineFromSamples(event.pathSamples,event.lineLengthM,event.preimage,event.genotype);currentGenotype=new Float64Array(event.genotype);currentCertificate=event.certificate;currentV2Representations=event.representations;currentProfile=profileFromPacked(event.profileNodes,event.edgeCount,event.lapTime,currentLine,event.lineLengthM);profileCursorTime=0;pausedRaceTime=0;profileMarkActive=false;scene.certifiedBest=currentLine;scene.provisionalBest=null;scene.invalidFlash=false;updateProfileDisplay();if(event.lapTime<priorLap-1e-6)runCertifiedImprovement=true;}
...
function handleCertifier(event:CertifierEvent):void{if(event.runVersion!==runVersion||event.trackFingerprint!==fingerprint||event.settingsFingerprint!==settingsFingerprint())return;if(event.type==="ready"){resolveCertifier?.();resolveCertifier=null;rejectCertifier=null;return;}
  if(event.type==="certified"){const job=activeCertification;if(!job||event.candidateId!==job.candidateId)return;activeCertification=null;if(event.certificate.pass&&job.source!=="baseline"){runCertifiedCandidates++;const elapsed=Math.max((performance.now()-runStarted)/1000,.001);$("certified-rate").textContent=(runCertifiedCandidates/elapsed).toFixed(2);}if(event.certificate.pass&&shouldAdoptCertifiedLap(event.lapTime,currentCertificate?currentProfile.lapTime:Infinity)){commitCertified(event,job.representations);if(optimizing)setStatus(`GPU running · certified ${job.source} ${event.lapTime.toFixed(3)} s`,"busy");else if(!optimizerStopped)setStatus(`Certified ${job.source} · ${event.lapTime.toFixed(3)} s binary64 profile`,"active");}void pumpCertification();}
  else if(event.type==="curvatureCertified"){const job=activeCertification;if(!job||event.candidateId!==job.candidateId)return;activeCertification=null;if(event.certificate.pass){runCertifiedCandidates++;const elapsed=Math.max((performance.now()-runStarted)/1000,.001);$("certified-rate").textContent=(runCertifiedCandidates/elapsed).toFixed(2);}if(event.certificate.pass&&shouldAdoptCertifiedLap(event.lapTime,currentCertificate?currentProfile.lapTime:Infinity)){commitCurvature(event);setStatus(`${optimizing?"GPU running · ":""}certified intrinsic curvature ${event.lapTime.toFixed(3)} s`,optimizing?"busy":"active");}void pumpCertification();}
...
function startOptimizer():void{if(optimizing||trackSwitching)return;stopOptimizer(true);runVersion++;resetCertifier();
...
function handleOptimizer(event:OptimizerEvent,source:Worker):void{if(source!==optimizer||event.runVersion!==runVersion||event.trackFingerprint!==fingerprint||event.settingsFingerprint!==settingsFingerprint())return;if(event.type==="ready"){if(event.adapterInfo!=="initializing"){const seed=`${activeSeedLo.toString(16).padStart(8,"0")}:${activeSeedHi.toString(16).padStart(8,"0")}`;setStatus(`${event.cpuFallback?"CPU fallback":"GPU"} · ${event.adapterInfo} · ${deterministicRun?"deterministic":`seed ${seed}`}`,"busy");}}
  ...
  else if(event.type==="provisionalBest"){queueCertification(event.candidateSpace,event.genotype,event.lapTime,event.preimage,event.representations,event.candidateKey);}
  else if(event.type==="warning"){setStatus(`${event.stage} stage skipped · ${event.message} · certifying discovery`,"busy");}
  else if(event.type==="stopped"){optimizing=false;stoppingRequested=false;optimizerStopped=true;scene.candidateLines=null;scene.candidateOffsets=null;optimizer=null;terminateWorker(source);setStatus(certifying?"Stopped · certifying discovery and curvature finalists":"Stopped · finalizing","busy");finishStoppedRun();syncButtons();}
...
function stopOptimizer(immediate=false):void{const worker=optimizer;if(!worker){if(immediate){optimizing=false;stoppingRequested=false;syncButtons();}return;}if(immediate){optimizer=null;terminateWorker(worker);optimizing=false;stoppingRequested=false;optimizerStopped=false;scene.candidateLines=null;scene.candidateOffsets=null;syncButtons();}else if(!stoppingRequested){stoppingRequested=true;worker.postMessage({runVersion,trackFingerprint:fingerprint,settingsFingerprint:JSON.stringify(vehicle),type:"stop"} satisfies OptimizerCommand);setStatus("Validating final candidates","busy");syncButtons();}}

function syncButtons():void{const action=$("optimize-button") as HTMLButtonElement;action.textContent=optimizing?"STOP":"OPTIMIZE";action.disabled=stoppingRequested||editBusy||trackSwitching;action.classList.toggle("optimize",!optimizing);action.classList.toggle("stop",optimizing);($("track-list").querySelectorAll("button") as NodeListOf<HTMLButtonElement>).forEach(b=>b.disabled=optimizing||editBusy||trackSwitching);($("new-track-button") as HTMLButtonElement).disabled=optimizing||editBusy||trackSwitching;($("track-editor-name") as HTMLInputElement).disabled=optimizing||editBusy||trackSwitching;($("track-editor-done") as HTMLButtonElement).disabled=optimizing||editBusy||trackSwitching;($("reset-button") as HTMLButtonElement).disabled=trackSwitching;($("settings-grid").querySelectorAll("input, select") as NodeListOf<HTMLInputElement|HTMLSelectElement>).forEach(control=>control.disabled=trackSwitching);($("play-button") as HTMLButtonElement).disabled=false;($("save-button") as HTMLButtonElement).disabled=!currentCertificate||optimizing||certifying||trackSwitching;$("play-button").textContent=playing?"Ⅱ PAUSE":"▶ PLAY";$("zoom-button").setAttribute("aria-pressed",String(zoomed));const overlay=$("work-overlay"),validating=stoppingRequested||(!optimizing&&certifying),work=validating?"VALIDATING":optimizing?"OPTIMIZING":certifying?"VALIDATING":"";overlay.hidden=work==="";overlay.classList.toggle("validating",validating);overlay.querySelector("span")!.textContent=work;scene.workLabel=null;}
